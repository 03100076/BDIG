...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  for (int i = 0; i < all_true_pos.size(); ++i) {
    ...
    map<int, float> APs;
    map<int, vector<float> > Recall; //ADD by chen
    map<int, vector<float> > Precision;  //ADD by chen
    ...
    ///////////ADD by chen////////////////////
    std::ofstream fout,fout_1;
    fout.open("/home/xd/RefineDet_PKG/models/PKG_setting/Recall/Recall.txt", std::ios_base::out);
    if (fout.is_open() )
    {
    	for (map<int, vector<float> >::iterator it = Recall.begin(); it != Recall.end(); it++)
    	{
		int label = it->first;
    		fout<<("label = ");
		fout<<(label);
    		fout<<('\n');
    		fout<<('\n');
    		for (vector<float>::iterator iit = it->second.begin(); iit != it->second.end(); iit++)
    		{
    			fout<<(*iit);
    			fout<<('\n');
    		}
    ...
    fout_1.open("/home/xd/RefineDet_PKG/models/PKG_setting/Precision/Precision.txt", std::ios_base::out);
    if (fout_1.is_open() )
    {
    	for (map<int, vector<float> >::iterator it = Precision.begin(); it != Precision.end(); it++)
    	{
		int label = it->first;
    		fout_1<<("label = ");
		fout_1<<(label);
